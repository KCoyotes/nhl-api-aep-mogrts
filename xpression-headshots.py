...
teamSelect = input("Enter the Tricode for an NHL Team: ")
...
folderPath = Path(str(teamSelect)).mkdir(parents=True, exist_ok=True)
folderPath
...
with alive_bar(len(uniqueIDs), length=30, bar='smooth', enrich_print=False) as bar:
    for player in uniqueIDs:
        ...
        if response.status_code == 200:
            ...
            file_path = Path(str(teamSelect) + '\\' + playerInfo['people'][0]['lastName'] + '_' + playerInfo['people'][0]['firstName'] + '.png')
...
print("\nDownloads Complete!\n\nAll available headshots were saved to " + '\\' + str(teamSelect) + '\\')
